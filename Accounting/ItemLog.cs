...
namespace Accounting
{
    public partial class ItemLog : Form
    {
        ...
        {
            string query = "SELECT itemName,trim(round(item_log.quantity,4))+0 AS quantity,item_log.mtype,item_log.date,stockStatus,item_log.status,CONCAT(firstname,' ',lastname) AS User,remarks from item,item_log LEFT OUTER JOIN administrativeassociate ON admin_ID = item_log.userID" +
            ...
            string query1 = "SELECT itemName,trim(round(item_log.quantity,4))+0 AS quantity,item_log.mtype,item_log.date,stockStatus,item_log.status,CONCAT(firstname,' ',lastname) AS User,remarks from item,item_log LEFT OUTER JOIN administrativeassociate ON admin_ID = item_log.userID" +
